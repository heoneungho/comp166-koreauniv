...
**269p~ 객체의 삶과 죽음** _요약_
  ...
  * 힙에서의 변수: 객체와 레퍼런스변수가 저장된다.
  * 스택에서의 변수: 지역변수와 메소드가 저장된다.
  * 생성자: 메소드의 상태를 처음 지정해줌.
  * 널 레퍼런스: 아무것도 없음을 의미한다.

**298p 집중토론** _요약_

  * 인스턴스 변수: 힙에 존재, 객체의 상태를 지정하는데 쓰임
  * 지역 변수: 스택에 존재, 메소드의 상태를 지정해주는데 쓰임
...
•정적 메소드를 선언하는 방법: static이라는 키워드 사용 static final: 초기화 한 이후에는 절대 그 값을 바꿀 수 없다.
