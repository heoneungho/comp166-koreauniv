...
public class MiniMiniMusicApp
{
	public static void main(String[] args)
	{
		MiniMiniMusicApp mini = new MiniMiniMusicApp();//mini 객체를 만듭니다
		mini.play();//play 메소드를 실행
	}//main end
	...
	public void play()
	{
		try
		{
			Sequencer player = MidiSystem.getSequencer();//Sequencer를 받아서 엽니다(그래야만 쓸 수 있습니다. Sequencer는 자동으로 열리지 않거든요.
			player.open();
			
			Sequence seq = new Sequence(Sequence.PPQ, 4);
			//Sequence를 생성하면서 Sequence를 생성자로 넘겨줍니다.
			
			Track track = seq.createTrack();//Sequence에 Track을 요청합니다. Track은 Sequence에 들어있고 미디 데이터는 Track에 들어있습니다.
			 ...
			 *이 부분은 거의 인스턴스 코드라고 보면 됩니다.
			 *중요한 부분은setMessage()메소드에 대한 인자와
			 *MidiEvent생성자에 대한 인자입니다.
			 */
...
public class MiniMusicCmdLine //첫번째
{
	public static void main(String[] args)
	{
		...
		if(args.length < 2 )//input숫자의 개수가 2개보다 작으면
			System.out.println("악기와 음 높이르 ㄹ지정하는 인자를 입력하세요.");
		else
		{
			int instrument = Integer.parseInt(args[0]);//악기 설정
			int note = Integer.parseInt(args[1]);//노트 설정
			mini.play(instrument,note);//play 메소드 실행하면서 인자로 넘겨줍니다.
	...
	public void play(int instrument, int note)
	{
		try
		{
			Sequencer player = MidiSystem.getSequencer();//시퀀서를 생성합니다.
			player.open();//open 하지 않으면 실행할 수 없습니다.
			Sequence seq = new Sequence(Sequence.PPQ,4);
			Track track = seq.createTrack();//track 을 생성합니다.
...
public class SimpleGui1{
        public static void main(String []args){
                JFrame frame = new JFrame(); // 프레임을 생성합니다.
                JButton button = new JButton("click me");  //버튼을 생성합니다.

                frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE); // X버튼을 누르면 창이 종료되도록 설정합니다.

                frame.getContentPane().add(button); // 프레임에 버튼을 추가해줍니다.
...
import javax.swing.*;
import java.awt.event.*;//Action Listener와 Action Event가 들어있는 패키지 사용위한 import문
...
public class SimpleGui1B implements ActionListener//인터페이스를 구현합니다.
{
	...
	public void go()
	{
		...
		button.addActionListener(this);//버튼에 액션 리스너를 집어넣습니다
	...
	public void actionPerformed(ActionEvent event)//ActionListener의 메소드 구현
	{
		button.setText("I've been clicked!");//버튼 클릭 시 버튼의 메시지를 바꿔줍니다.
	}
}
...
public class SimpleGui3C implements ActionListener {
        ...
        public static void main(String[] args) {
                SimpleGui3C gui = new SimpleGui3C();//객체 생성
                gui.go();//go 메소드 호출
        }
        ...
        public void go() {
                ...
                JButton button = new JButton("Change colors");//버튼 생성 후 버튼에 메시지 넣음
                button.addActionListener(this);//액션 리스너 구현
                
                MyDrawPanel drawPanel = new MyDrawPanel();//패널 생성
                
                frame.getContentPane().add(BorderLayout.SOUTH,button);//버튼을 추가함
                frame.getContentPane().add(BorderLayout.CENTER,drawPanel);//패널을 추가함
                frame.setSize(300,300);//사이즈 설정
                frame.setVisible(true);//눈에 보이게
        }
        public void actionPerformed(ActionEvent event) {
                frame.repaint();//액션이 감지되면 실행
        }
}
...
class MyDrawPanel extends JPanel
{
	...
	{
		Graphics2D g2d = (Graphics2D) g;//타입 캐스트로 호출함
		...
		Color startColor = new Color(red,green,blue);//시작 컬러
...
public class TwoButtons
{
	...
	class LabelListener implements ActionListener//이너 클래스입니다.
	{
...
public class SimpleAnimation 
{
        ...
        public void go()
        {
                ...
                MyDrawPanel drawPanel = new MyDrawPanel();// 위젯을 만들고 프레임에 집어 넣습니다.

                frame.getContentPane().add(drawPanel);
                frame.setSize(300,300);
                frame.setVisible(true);

                for(int i=0;i<300;i++){ // 130번 반복합니다.

                        x++;
                        y++;
                        //x와 y 좌표를 증가시킵니다.
                        
                        drawPanel.repaint(); // 스스로 다시 칠하라고 패널에게 말합니다.
		...
		class MyDrawPanel extends JPanel//내부 클래스에요
        {
                public void paintComponent(Graphics g)
                {
                        g.setColor(Color.white);
                        g.fillRect(0,0,this.getWidth(),this.getHeight());//지나온 자리를 지워줘요

                        g.setColor(Color.green);
                        g.fillOval(x, y, 40, 40); //지속적으로 갱신되는 외부클래스의 x, y 좌표를 사용합니다.
...
public class MiniMusicPlayer3 
{
        ...
        public static void main(String [] args)
        {
                MiniMusicPlayer3 mini = new MiniMusicPlayer3();//객체를 생성합니다.
                mini.go();//go메소드 호출
        }// 메소드 끝

        public void setUpGui()
        {
                m1 = new MyDrawPanel();//패널 만들고
                f.setContentPane(m1);//집어넣고
...
public class BeatBox
{
    ...
    String[] instrumentNames = {"Bass Drum", "Closed Hi-Hat", 
       ...
       "Open Hi Conga"}; 
    // GUI 레이블을 만들 때 사용할 악기명 String 배열로 저장합니다.
    ...
    public void buildGUI() 
    {
        ...
        // 비어있는 경계선을 사용하여 패널 둘레와 구성요소가 들어가는 자리 사이에 빈 공간을 만들 수 있습니다.
        ...
        //체크상자를 만들고 모든 값을 'false'로 설정한 다음 arraylist와 gui패널에 추가합니다.
        //순환문 끝
    ...
    public void buildTrackAndStart()
    {
    	...
    	int[] trackList = null; //각 악기의 열여섯 박자에 대한 값을 배열에 저장합니다.
    
    	sequence.deleteTrack(track);
    	track = sequence.createTrack();
    	//기존 트랙을 제거하고 트랙을 새로 만듭니다.

	    	for (int i = 0; i < 16; i++) { // 열 16개에 대해 모두 같은 작업을 처리합니다.
	          ...
	          {         
	              ...
	              //이 박자에 해당하는 체크상자가 선택되었는지 확인합니다. 그렇다면 배열에 건반을 집어넣고 아니면 0으로 설정
	              
	           } // 안쪽 for문 끝
	         
	           makeTracks(trackList); //이 악기의 16개의 모든 박자에 대해 이벤트를 만들고 트랙에 추가합니다.
...
public class BeatBox
{
    ...
    {
    	@Override
    	public void actionPerformed(ActionEvent e)//사용자가 버튼을 클릭해서 ActionEvent가 발생된 경우에 실행됩니다.
    	{
    		...
    		//checkboxlist를 훑어보면서 각 체크상자의 상태를 확인하고 배열에 추가합니다.
    ...
    public class MyReadInListener implements ActionListener
    {
    	...
    	public void actionPerformed(ActionEvent e)
    	{
    		...
    		try
    		{
    			...
    			checkboxState = (boolean[]) is.readObject();//파일에서 객체 하나를 읽은 다음 부울 배열로 캐스트합니다.
    		...
    		//각각의 체크상자를 원래대로 복구합니다.
    		
    		sequencer.stop();
    		buildTrackAndStart();
    		//현재 연주중인 것을 멈추고 ArrayList에 들어있는 체크상자의 새로운 상태를 이용하여 시퀀스를 재구성합니다.
    ...
    public class MyStartListener implements ActionListener 
    {
      ...
      //복붙한다면 멍청이같이 이 주석을 똑같이 붙여넣겠지?-근영
    public class MyStopListener implements ActionListener 
    {
        public void actionPerformed(ActionEvent a) 
        {
