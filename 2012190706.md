...
public class SimpleGui3C implements ActionListener { //인터페이스를 구현합니다. 이 구문은 SimpleGui3C의 객체는 ActionListener 임을 나타냅니다.

	
	JFrame frame; // JFrame을 만듭니다.

	public static void main(String [] args){
		SimpleGui3C gui=new SimpleGui3C(); // gui 라는 SimpleGui3C의 객체를 만듭니다.
		gui.go(); // go 매소드를 실행시킵니다.
	}

	public void go() {
		frame=new JFrame(); //JFrame을 만듭니다.
		frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);  // 이렇게 하면 창을 닫았을 때 프로그램이 바로 종료됩니다.


		JButton button = new JButton("Change colors"); // JButton을 만드는데 그 버튼은 Change colors 를 나타냅니다.
		button.addActionListener(this); // 리스너(this)를 button에 추가합니다.

		MyDrawPanel drawPanel = new MyDrawPanel(); // drawPanel이라는 MyDrawPanel 객체를 만듭니다.

		frame.getContentPane().add(BorderLayout.SOUTH,button); // button 을 south 지역에 추가합니다.
		frame.getContentPane().add(BorderLayout.CENTER, drawPanel); // drawPanel 을 center 지역에 추가합니다.
		frame.setSize(300,300); // 프레임의 크기를 픽셀 단위로 지정합니다.
		frame.setVisible(true); // 화면에 표시되도록 합니다.
	}
	public void actionPerformed(ActionEvent event){
		frame.repaint(); //사용자가 버튼을 클릭하면 프레임의 repaint()메소드를 호출합니다. 그러면 그 프레임에 들어잇는 모든 위젯의 paintComponent() 메소드가 호출됩니다.
	}
}

// 그래디언트의 시작과 끝 색을 무작위로 지정하게 만든 것입니다. 
...
public class TwoButtons { //TwoButtons 클래스를 만듭니다.

	JFrame frame; //frame 이라는 JFrame 변수를 만듭니다.
	JLabel label; //lable 이라는 JLable 변수를 만듭니다.

	public static void main(String args[]){
		TwoButtons gui = new TwoButtons(); //gui 라는 TwoButtons 의 객체를 만듭니다.
		gui.go(); //go메소드를 실행시킵니다.
	}
	...
	public void go(){
		...
		JButton labelButton = new JButton ("Change Label"); //JButton을 만들고 그 버튼은 Change Label 을 나타냅니다.

		labelButton.addActionListener(new LabelListener()); //버튼의 리스너 등록 메소드에 this가 아닌 다른 리스너 클래스를 가지고 만든 객체를 전달합니다

		JButton colorButton = new JButton ("Change Circle"); //JButton을 만들고 그 버튼은 Change Circle 을 나타냅니다.

		colorButton.addActionListener(new ColorListener()); //버튼의 리스너 등록 메소드에 this가 아닌 다른 리스너 클래스를 가지고 만든 객체를 전달합니다

		label = new JLabel("I'm a lebel"); //JLabel을 만들고 그 라벨은 I'm a label 을 나타냅니다.
		MyDrawPanel drawPanel = new MyDrawPanel(); //drawPanel 이라는 McDrawPanel 의 객체를 만듭니다.


		frame.getContentPane().add(BorderLayout.SOUTH, colorButton); //colorButton은 판에서 south 지역 입니다.
		frame.getContentPane().add(BorderLayout.CENTER, drawPanel); //drawPanel은 판에서 center 지역 입니다.
		frame.getContentPane().add(BorderLayout.EAST, labelButton); //labelButton은 판에서 east 지역 입니다.
		frame.getContentPane().add(BorderLayout.WEST, label); //label은 판에서 west 지역 입니다.
	...
	class LabelListener implements ActionListener{ //LabelKistencer 이름을 갖는 액션리스너 입니다.
		public void actionPerformed(ActionEvent event){
			label.setText("Ouch!"); // 내부 클래스에서도 'label' 변수를 사용할 수 있습니다.
		}
	} // 내부 클래스 끝

	class ColorListener implements ActionListener{ //ColorListener 이름을 갖는 액션리스너 입니다.
		public void actionPerformed(ActionEvent event){
			frame.repaint(); // 내부 클래스에서는 외부 클래스 객체에 대한 레퍼런스 없이 그냥 'frame' 인스턴스 변수를 사용하면 됩니다.
		}
	} // 내부 클래스 끝
}
...
// 메인 GUI 클래스에 원의 x와 y좌표를 저장하기 위한 인스턴스 변수 두개를 만듭니다.
	int x=70; //x라는 int 변수에 70을 넣습니다.
	int y=70; //y라는 int 변수에 70을 넣습니다.
	

	public static void main(String args[]){
		SimpleAnimation gui=new SimpleAnimation(); //gui라는 SimpleAnimation 의 객체를 만듭니다.
		gui.go(); //go 메소드를 실행시킵니다.
	}
	public void go(){
		JFrame frame=new JFrame(); //JFrame을 만듭니다.
		frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE); // 이렇게 하면 창을 닫았을 때 프로그램이 바로 종료됩니다.

// 위젯을 만들고 프레임에 집어넣기만 하면 됩니다.
		MyDrawPanel drawPanel = new MyDrawPanel(); 

		frame.getContentPane().add(drawPanel); //판에 drawPanel 을 추가시킵니다.
		frame.setSize(300,300); // 프레임의 크기를 픽셀 단위로 지정합니다.

		frame.setVisible(true); // 화면에 표시되도록 합니다.

		for(int i=0;i<300;i++){ // for 순환문을 이용해 130번 반복합니다.

			x++; // x를 1 증가시킵니다.
			y++; // y를 1 증가시킵니다.

			drawPanel.repaint(); // 스스로 다시 칠하라고 패널에게 말합니다. 그러면 새로운 위치에 있는 원을 볼 수 있습니다.
...
// 내부 클래스 부분입니다.
	class MyDrawPanel extends JPanel{ // JPanel 을 상속받습니다.
		private static final long serialVersionUID = 1L;
		public void paintComponent(Graphics g){
			g.setColor(Color.green);
			g.fillRect(0,0,this.getWidth(),this.getHeight()); //getWidth()와 getHeight()는 JPanel 로부터 상속 받은 메소드입니다.

			g.setColor(Color.green);
			g.fillOval(x, y, 40, 40); //지속적으로 갱신되는 외부 클래스의 x와 y좌표를 사용합니다.
...
public class MiniMusicPlayer3 {
	...
	static JFrame f = new JFrame("My First Music Video"); //JFrame을 만들고 My First Music Video 가 뜨게 합니다.
	static MyDrawPanel m1; 

	public static void main(String [] args){
		MiniMusicPlayer3 mini = new MiniMusicPlayer3(); //mini이라는 MiniMusicPlayer3 객체를 만듭니다.
		mini.go(); //go 메소드를 실행합니다.
	}// 메소드 끝

	public void setUpGui(){
		m1 = new MyDrawPanel(); //m1이라는 레퍼런스변수에 MyDrawPane1 에 연결시킵니다.
		f.setContentPane(m1); //m1 을 판에 추가시킵니다.
		f.setBounds(30, 30, 300, 300); //위치를 설정합니다.
		f.setVisible(true);  //화면에 표시되도록 합니다.

	}// 메소드 끝

	public void go(){
		setUpGui(); //setUpGui 메소드를 실행시킵니다.

		try{

			Sequencer sequencer = MidiSystem.getSequencer(); //sequencer 를 만듭니다.
			sequencer.open(); // sequencer를 엽니다.
			sequencer.addControllerEventListener(m1, new int[] {127}); //이벤트를 sequencer에 등록합니다.
			Sequence seq = new Sequence(Sequence.PPQ, 4); //seq 라는 sequence 객체를 하나 만듭니다.
			...
			}// 순환문 끝, 박자를 골라내기 위한 코드입니다.

			sequencer.setSequence(seq);
			sequencer.start();
			sequencer.setTempoInBPM(120);
		} catch(Exception ex) {ex.printStackTrace();} //예외 발생시 printStackTrace()를 실행시킵니다.
	}// 메소드 끝

	public MidiEvent makeEvent(int comd, int chan, int one, int two, int tick){ //매개변수가 5개입니다.
		MidiEvent event = null; //event 를 null로 만듭니다. 
		try{
			ShortMessage a = new ShortMessage(); //메세지 인스턴스를 만듭니다.
			a. setMessage(comd,chan,one,two); //setMessage()를 호출하여 지시사항을 전달합니다.
			event = new MidiEvent(a, tick); //메세지에 대한 MidiEvent 인스턴스를 만듭니다.
		}catch(Exception e){}
		return event; //이벤트를 리턴합니다.

	}// 메소드 끝

	class MyDrawPanel extends JPanel implements ControllerEventListener{ //MyDrawPanel이 리스너입니다.
		boolean msg=false; //플래그를 false로 설정한 다음 이벤트를 받을 때만 true로 설정합니다.

		public void controlChange(ShortMessage evet){
			msg= true; //이벤트를 받았으므로 플래그를 true로 설정합니다.
			repaint(); //repaint()를 호출합니다.
		}

		public void paintComponent(Graphics g){
			if(msg){ //다른 이류로 인해 repaint()가 호출될 수도 있는데, ControllerEvent가 발생한 경우에만 그림을 다시 그려야 하므로 플래그를 써야 합니다.

				Graphics2D g2= (Graphics2D)g;

				int r = (int) (Math.random()*250); //RGB의 R에 들어갈 색 숫자를 랜덤으로 뽑습니다.
				int gr=(int) (Math.random()*250); //RGB의 G에 들어갈 색 숫자를 랜덤으로 뽑습니다.
				int b= (int ) (Math.random()*250); //RGB의 B에 들어갈 색 숫자를 랜덤으로 뽑습니다.
				
				g.setColor(new Color(r,gr,b)); //뽑힌 R,G,B 숫자를 저장합니다.

				int ht=(int) ((Math.random()*120)+10); //높이에 들어갈 수치를 랜덤으로 뽑습니다.
				int width=(int) ((Math.random()*120)+10); //너비에 들어갈 수치를 랜덤으로 뽑습니다.

				int x=(int) ((Math.random()*40)+10);
				int y=(int) ((Math.random()*40)+10);

				g.fillRect(x, y, ht, width); //직사각형을 만들기 위해 값을 설정합니다.
				msg = false;
				//무작위적으로 색을 생성한 후 거의 무작위적으로 직사각형을 그리기 위한 코드입니다.
			}//if문 끝
		}// 메소드 끝
	}// 내부 클래스 끝
}//클래스 끝
...
public class BeatBox {
	...
	JPanel mainPanel; //JPanel 을 만듭니다.
	ArrayList<JCheckBox> checkboxList; // 체크상자를 ArrayList에 저장합니다.
	Sequencer sequencer; //Sequencer를 만듭니다.
	Sequence sequence; //Sequence를 만듭니다.
	Track track; //Track 을 만듭니다.
	JFrame theFrame; //JFrame 을 만듭니다.

// GUI 레이블을 만들 때 사용할 악기명을 String 배열로 저장합니다.
	...
	int []instruments ={35,42,46,38,49,39,50,60,70,72,64,56,58,47,67,63}; // 실제 드럼'건반'을 나타냅니다. 드럼 채널은 피아노의 각 건반이 서로 다른 드럼을 나타내는 것과 같다고 보면 됩니다. 35번 건반은 베이스 드럼을, 42번 건반은 하이햇(H-Hat)을 나타내는 것을 말합니다.

	public static void main(String [] args){
		new BeatBox().buildGUI(); //buildGUI 메소드를 실행시킵니다.
	}

	public void buildGUI(){
		theFrame = new JFrame("Cyber BeatBox"); // JFrame을 만들고 Cyber BeatBox를 나타냅니다.
		theFrame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE); // 이렇게 하면 창을 닫았을 때 프로그램이 바로 종료됩니다.
		BorderLayout layout = new BorderLayout(); //layout 이라는 BorderLayout 객체를 만듭니다.
		JPanel background = new JPanel(layout); //background 라는 JPanel 객체를 만듭니다. 
		background.setBorder(BorderFactory.createEmptyBorder(10,10,10,10)); //비어있는 경계선(empty border)을 사용하여 패널 둘레와 구성요소가 들어가는 자리 사이에 빈 공간을 만들 수 있습니다.

		checkboxList = new ArrayList<JCheckBox>(); //checkboxList라는 어레이 리스트를 만듭니다. 이 어레이 리스트는 JCheckBox객체를 담습니다.
		Box buttonBox = new Box(BoxLayout.Y_AXIS); //buttonBox라는 Box객체를 만듭니다.


		JButton start = new JButton("Start"); //start 라는 JButton 을 만을고 그위에 Start를 나타내게 합니다.
		start.addActionListener(new MyStartListener()); //start에 액션리스너를 추가시킵니다.
		buttonBox.add(start); //buttonBox에 start를 넣습니다.

		JButton stop = new JButton("Stop"); //stop 이라는 JButton 을 만들고 그 위에 Stop를 나타내게 합니다.
		stop.addActionListener (new MyStopListener()); //stop에 액션리스너를 추가시킵니다.
		buttonBox.add(stop); //buttonBox에 stop을 넣습니다.

		JButton upTempo = new JButton("upTempo"); //upTempo라는 JButton 을 만들고 그 위에 upTempo를 나타내게 합니다.
		upTempo.addActionListener(new MyUpTempoListener()); //upTempo에 액션리스너를 추가시킵니다.
		buttonBox.add(upTempo, "upTempo"); //buttonBox에 upTempo를 넣습니다.

		JButton downTempo = new JButton("Tempo Down"); //downTempo라는 JButton 을 만들고 그 위에 Tempo Down을 나타내게 합니다.
		downTempo.addActionListener(new MyDownTempoListener()); //downTempo에 액션리스너를 추가시킵니다.
		buttonBox.add(downTempo, "downTempo"); //buttonBox에 downTempo를 넣습니다.

		Box nameBox = new Box(BoxLayout.Y_AXIS); //nameBox라는 Box객체를 만듭니다. 이것들은 y축 기준으로 배치됩니다. 
		for(int i=0; i<16; i++){
			nameBox.add(new Label(instrumentNames[i])); //for문을 이용해 nameBox에 악기이름을 추가시킵니다.
		}


		background.add(BorderLayout.EAST, buttonBox); //buttonBox의 위치를 east 지역으로 합니다.
		background.add(BorderLayout.WEST, nameBox); //nameBox의 위치를 west 지역으로 합니다.

		theFrame.getContentPane().add(background); //판에 background를 추가시킵니다.

		GridLayout grid = new GridLayout(16,16); //grid란른 GridLayout 객체를 만듭니다.
		grid.setVgap(1);
		grid.setHgap(2);
		mainPanel = new JPanel(grid); //JPanel 을 만듭니다.
		background.add(BorderLayout.CENTER,mainPanel); //mainPanel 의 위치를 center지역으로 합니다.
		...
		for(int i=0;i<256;i++)
		{
			...
			checkboxList.add(c);
			mainPanel.add(c);
		}//순환문 끝, 체크상자를 만들고 모든값을 'false'로 설정한 다음 ArrayList와 GUI 패널에 추가합니다.

		setUpMidi(); //setUpMidi메소들를 실행시킵니다.

		theFrame.setBounds(50,50,300,300);
		theFrame.pack(); //pack 메소드를 실행시킵니다.
		theFrame.setVisible(true); // theFrame이 보이도록 합니다.    
	...
	public void setUpMidi(){
		try{
			sequencer= MidiSystem. getSequencer(); //시퀀서를 받습니다.
			sequencer.open(); //시퀀서를 엽니다.
			sequence = new Sequence(Sequence.PPQ,4);
			track = sequence.createTrack(); //트랙을 만듭니다.
			sequencer.setTempoInBPM(120);
		}catch(Exception e){e.printStackTrace();}
	}//setUpMidi 메소드 끝

// 체크상자의 상태를 MIDI이벤트로 바꾼 다음 그 이벤트를 트랙에 추가합니다.
	public void buildTrackAndStart(){
		int [] trackList = null; // 각 악기의 열여섯 박자에 대한 값을 원소가 16개인 배열에 저장합니다. 어떤 악기가 특정 박자에서 연주되어야하면 그 원소의 값에 건반 번호를 넣습니다. 반대로 연주되어야 하지 않는다면 0을 집어넣습니다.

		sequence.deleteTrack(track); //기존 트랙을 제거합니다.
		track = sequence.createTrack(); //새로운 트랙을 만듭니다.


		for(int i = 0; i< 16; i++){
			trackList = new int[16]; //열 16개 모두에 대해 같은 작업을 처리합니다.

			int key = instruments[i]; // 어떤 악기인지를 나타내는 건반 번호를 설정합니다.
			...
			} //안쪽 for문 끝, 박자에 해당하는 체크상자가 선택되었는지 확인합니다. 선택되어 있으면 배열의 그 자리에 건반값을 집어넣습니다. 그렇지 않으면 그 박자에서는 그 악기를 연주하지 않아야 하므로 0으로 설정합니다.

			makeTracks(trackList); //악기의 16개의 모든 박자에 대해 이벤트를 만듭니다.
			track.add(makeEvent(176,1,127,0,16)); //트랙에 추가합니다.
		}// 바깥쪽 for문 끝

		track.add(makeEvent(192,9,1,0,15)); //16번째 박자에는 반드시 이벤트가 있어야 합니다. 이렇게 하지 않으면 다시 시작하기 전에 16박자가 모두 끝나지 않을 수도 있습니다.
		try{
			sequencer.setSequence(sequence);
			sequencer.setLoopCount(sequencer.LOOP_CONTINUOUSLY); //루프 반복 회수를 지정하기 위한 메소드입니다.
			sequencer.start(); //start메소드를 실행합니다.
			sequencer.setTempoInBPM(120);
		}catch(Exception e){e.printStackTrace();}
	} // buildTrackAndStart 메소드 끝

	public class MyStartListener implements ActionListener { //Start 버튼의 액션리스너입니다.
		public void actionPerformed(ActionEvent a){
			buildTrackAndStart();
		}
	} // 내부 클래스 끝 ,첫번째 내부 클래스.버튼의 리스너 입니다.

	public class MyStopListener implements ActionListener{ //Stop 버튼의 액션리스너입니다.
		public void actionPerformed(ActionEvent a){
			sequencer.stop();
		}
	}// 내부 클래스 끝

	public class MyUpTempoListener implements ActionListener{ //Tempo Up 버튼의 액션리스너입니다.
		public void actionPerformed(ActionEvent a){
			float tempoFactor = sequencer.getTempoFactor();
			sequencer.setTempoFactor((float)(tempoFactor * 1.03)); //시퀀서의 빠르기를 주어진 배율을 가지고 변경합니다. 빠르기를 3%증가시켰습니다.
		}
	}// 내부 클래스 끝
	public class MyDownTempoListener implements ActionListener { //Tempo Down 버튼의 액션리스너입니다.
		public void actionPerformed(ActionEvent a){
			float tempoFactor = sequencer.getTempoFactor();
			sequencer.setTempoFactor((float)(tempoFactor* .97)); //시퀀서의 빠르기를 주어진 배율을 가지고 변경합니다. 빠르기를 3%감소시켰습니다.
		}
	}// 내부 클래스 끝

	

	public void makeTracks(int [] list){ // 한 악기의 16박자 전체에 대해 이벤트를 만듭니다. int[]배열에는 그 악기에 해당하는 건반 번호 또는 0이 들어갑니다. 0이 들어있다면 그 박자에서는 해당 악기가 연주되지 않아야 합니다. 0이 아닌 값이 들어있다면 이벤트를 만들어서 트랙에 추가합니다.
		for(int i=0;i<16;i++){
			int key = list[i];
...
public class MySendListener implements ActionListener {
	public void actionPerformed(ActionEvent a) { //사용자가 버튼을 클릭해서 ActionEvent가 발생된 경우에 실행됩니다.
		...
		} // checkboxList(체크상자로 이루어진 ArrayList)를 훑어보면서 각 체크상자의 상태를 확인하고 그 결과를 부울 배열에 추가합니다.
...
public class MyReadInListener implements ActionListener {
	public void actionPerformed(ActionEvent a) {
		...
		try {
			...
			checkboxState = (boolean[]) is.readObject(); // 파일에서 객체하나를 읽은 다음 부울 배열로 다시 캐스트 합니다.
		...
		} //실제 JCheckBox 객체로 구성된 ArrayList(checkobxList)에 들어있는 각각의 체크상자의 상태를 원래대로 복구합니다.
		
		sequencer.stop(); //현재 연주중인 것을 멈춥니다.
		buildTrackAndStart(); // ArrayList에 들어있는 체크상자의 새로운 상태를 이용하여 시퀀스를 재구성합니다.
