...
public class SimpleGui1 {
	public static void main(String[] args){
		...
		JFrame frame = new JFrame(); // 프레임과 버튼을 만듭니다.
		JButton button = new JButton("click me");
		
		frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE); // 이러게 하면 창을 닫았을 때 프로그램이 바로 종료됩니다.
		
		frame.getContentPane().add(button); // 버튼을 프레임의 내용 틀에 추가
		
		frame.setSize(300,300); // 프레임의 크기를 픽셀단위로 지정
		
		frame.setVisible(true); // 마지막으로 화면에 표시되도록 설정
	}
...
public class SimpleGui1B implements ActionListener { // 인터페이스를 구현
	JButton button;
	...
	public void actionPerformed(ActionEvent event){ // 이 메소드를 호출하여 이벤트가 일어났다는 것을 알려준다.
...
class MyDrawPanel extends JPanel{     
	public void paintComponent(Graphics g) {              
		...
		Color startColor = new Color(red,green,blue); // 시작했을때의 색깔           
		red = (int) (Math.random() * 256);   
		green = (int) (Math.random() * 256);       
		blue = (int) (Math.random() * 256);          
		Color endColor = new Color(red,green,blue); // 클릭 후 변하는 색깔     
...
public class TwoButtons{  
	...
	public void go() {  
		...
		labelButton.addActionListener(new LabelListener()); // 버튼의 리스너 등록 메소드에 this가 아닌 다른 리스너 클래스를 가지고 만든 객체를 전달 
		JButton colorButton = new JButton("Change Circle");     
		colorButton.addActionListener(new ColorListener());// 버튼의 리스너 등록 메소드에 this가 아닌 다른 리스너 클래스를 가지고 만든 객체를 전달      
...
class LabelListener implements ActionListener{ // 클래스 하나 안에 ActionListener 두개를 집어넣는다.
        public void actionPerformed(ActionEvent event)  {     
        	label.setText("Ouch!"); // text 변경         
        	}               
        }    

class ColorListener implements ActionListener {  // 클래스 하나 안에 ActionListener 두개를 집어넣는다.
	public void actionPerformed(ActionEvent event){  
...
public class SimpleAnimation { 
	...
	public void go() {
		...
		MyDrawPanel drawPanel = new MyDrawPanel(); // 위젯을 만들고 프레임에 집어 넣기만 하면 됩니다..  
...
public class MiniMusicPlayer3 {   
	...
	static JFrame f = new JFrame("My First Music Video"); // 프레임을 생성 합니다.  
	static MyDrawPanel m1; // 패널을 생성합니다.     
	...
	public void setUpGui() {        
		m1 = new MyDrawPanel(); // 패널을 만듭니다.
		f.setContentPane(m1); // 패널을 집어넣습니다.          
		f.setBounds(30, 30, 300, 300); // 경계를 설정합니다.  
...
public class BeatBox{
	...
	int[] instruments = {35,42,46,38,49,39,50,60,70,72,64,56,58,47,67,63}; // 실제 드럼 '건반'을 나타냅니다. 드럼 채널은 피아노의 각 건반이 서로 다른 드럼을 나타내는 것과 같다고 보면 됩니다.   
	...
	public void buildGUI() {   
		...
		for (int i = 0; i < 256; i++) { // 체크상자를 만들고 모든 값을 'false'로 설정한 다음 arraylist와 gui패널에 추가합니다.
	...
	public void buildTrackAndStart() {   
		int[] trackList = null; // 각 악기의 열여섯 박자에 대한 값을 원소가 16개인 배열에 저장합니다.     
		...
		for (int i = 0; i < 16; i++) { // 열 16개에 대해 모두 같은 작업을 처리합니다.     
			...
			for (int j = 0; j < 16; j++ ) { // 이 열에 들어있는 모든 박자에 대해 같은 작업을 반복합니다.
				
				JCheckBox jc = (JCheckBox) checkboxList.get(j + (16*i));      
				if ( jc.isSelected()){ // 이 박자에 해당하는 체크상자가 선택되었는지 확인합니다. 그렇다면 배열에 건반을 집어넣고 아니면 0으로 설정합니다. 
					trackList[j] = key;    
					} else {    
						trackList[j] = 0;    
						}                         
...
public class MySendListener implements ActionListener{
	...
	public void actionPerformed(ActionEvent a){ // 사용자가 버튼을 클릭해서 ActionEvent가 발생된 경우에 실행됨
		boolean[] checkboxState = new boolean[256]; // 각 체크상자의 상태를 담아두기 위한 부울 배열을 만든다.
