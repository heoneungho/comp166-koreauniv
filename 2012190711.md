...
**199p~ 상속과 다형성** _요약_
  ...
  * 상속: 하위클래스가 상위클래스의 멤버(인스턴스변수, 메소드)를 물려받는 것
  * 다형성: 레퍼런스 유형을 실제 객체 유형의 상위클래스 유형으로 지정할 수 있다
> > 다형적인 인자를 사용하는 코드를 만들면, 즉 메소드 매개변수를 상위클래스 유형으로 선언하면 실행할 때 어떤 하위클래스의 객체도 전달할 수 있다.
...
public class Animal {
        String picture; //동물의 사진
        String food; //먹는 음식의 형식
        int hunger; //배고픈 정도
        int boundaries; //돌아다니는 공간
        int location; // 동물의 위치
        
        public void makeNoise(){} //소리를 낼 때의 행동
        public void eat(){} //음식을 접했을 때의 행동
        public void sleep(){} //잠들어 있을 때의 행동
        public void roam(){} //돌아다닐 때의 행동
...
**224p 오버라이드와 오버로딩** _요약_
  ...
  * 오버라이드: 상속되는 메소드를 인자와 리턴형식은 같게하여 메소드의 설정을 바꾸는것
...
public class MonsterTestDrive
{
        public static void main(String[] args)
        {
                Monster[] ma = new Monster[3];//Monster 객체배열

                ma[0] = new Vampire();
                ma[1] = new Dragon();
                ma[2] = new Monster(); //각자 넣어줌

                for(int x = 0; x < 3; x++)
                {
                        ma[x].frighten(x);//순서대로 frighten 메소드 호출
                }
        }
}
...
class Vampire extends Monster//Monster를 상속
{
...
class V2Radiator
{
        V2Radiator(ArrayList<SimUnit> list)//생성자로 add메소드 호출
        {
                for(int x = 5; x < 5 ; x++)
                {
...
class SimUnit //전력 소모량의 차이의 문제는 생성될 때마다 메시지를 출력하라는 생성자를 만들면 문제를 빨리 찾을 수 있다.
{
        String botType;
        SimUnit(String type)
        {
