...
public class BeatBox {
	...
	String[] instrumentNames={"Bass Drum", "closed hi-hat", "open hi-hat", "acoustci snare", "crash symbal", "hand clap", "high tom", "hi bongo", "maracas", "whistle", "low conga", "cowbell", "vibraslap", "low-mid tom", "high agogo", "open hi conga"};
	...
	public void buildGUI(){
		theFrame= new JFrame("Cyber beatbox");//프레임 생성
		...
		JButton start = new JButton("start");//start버튼 생성해서 추가
		start.addActionListener(new MyStartListener());
		buttonBox.add(start);
		
		JButton stop= new JButton("stop");//stop버튼 생성해서 추가
		start.addActionListener(new MyStopListener());
		buttonBox.add(stop);
		
		JButton upTempo= new JButton("Tempo up");//Tempo up버튼 생성해서 추가
		start.addActionListener(new MyStopListener());
		buttonBox.add(upTempo);	
		
		JButton downTempo= new JButton("Tempo down");//Tempo down버튼 생성해서 추가
		...
		background.add(BorderLayout.EAST,buttonBox);//BorderLayout 오른쪽에 buttonBox추가
		background.add(BorderLayout.WEST,nameBox);//BorderLayout 왼쪽에 nameBox추가
	...
	public void setUpMidi(){//sequencer, track 생성
		try{
	...
	public void buildTrackAndStart(){
		...
		for(int i=0;i<16;i++){
			...
			makeTracks(trackList);//이벤트를 트렉에 추가
		...
		try{
			...
			sequencer.setLoopCount(sequencer.LOOP_CONTINUOUSLY);//루프 반복횟수 저장
			sequencer.start();
			sequencer.setTempoInBPM(120);
		}catch(Exception e){e.printStackTrace();}
	}
	
	public class MyStartListener implements ActionListener{//리스너 구현
...
public class BeatBox {
	...
	public class MySendListener implements ActionListener{
		public void actionPerformed(ActionEvent a){//버튼 클릭후 수행부분
			boolean[] checkboxState=new boolean[256];//체크상자의 체크여부
			...
			try{
				FileOutputStream fileStream=new FileOutputStream(new File("checkbox.ser"));a//직렬화하여 저장
	...
	public class MyReadInListener implements ActionListener{
		public void actionPerformed(ActionEvent a){
			...
			for(int i=0;i<256;i++){//체크상자의 상태를 원래대로 복구
				JCheckBox check=(JCheckBox)checkboxList.get(i);
				if(checkboxState[i]){
