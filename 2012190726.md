...
public class DotComBust{
	...
	private GameHelper helper = new GameHelper(); //사용할 변수를 선언하고 초기화 합니다.
	...
	private void setUpGame(){ //우선 닷컴 객체 몇 개를 만들고 위치를 지정합니다.
		...
		for(DotCom dotComToSet : dotComsList){
			...
			dotComToSet.setLocationCells(newlocation); //DotCom 객체의 세터 메소드를 호출하여 방금 보조 메소드에서 받은 위치를 지정.
		} //for 순환문 끝
	} //setUpgame 메소드 끝
		...
		int index = locationCells.indexOf(userInput);
			if(index >= 0){
				...
				if(locationCells.isEmpty()){
					status = "kill";
					System.out.println("Ouch! You sunk " +name+ " : ( "); //닷컴 으앙쥬금
				}else{
					status ="hit";
				}
			} //if문 끝
			
		return status; //결과 값(miss,hit,kill)을 리턴
	} //메소드 끝
} //클래스 끝
